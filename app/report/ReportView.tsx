...
/**
 * Presentational report scorecard. No hooks, no data fetching — pure props —
 * so it renders identically for a live session, the static example, and the
 * shareable URL version (CLAUDE.md rule 11).
 */
...
const TIER_LABEL: Record<"surface" | "probe" | "deep", string> = {
  surface: "Surface",
  probe: "Probe",
  deep: "Deep",
};
    ...
    <article className="mx-auto w-full max-w-2xl px-6 py-12">
      ...
      <header className="flex items-end justify-between gap-6 border-b border-ink-800 pb-8">
        ...
        <div className="hidden w-56 shrink-0 sm:block">
          ...
          <Stat label="Technique" value={scores.technique} max={50} />
        </div>
      </header>
          ...
          <ul className="mt-5 space-y-5">
            {narrative.missed.map((m) => (
              ...
              >
                ...
                <p className="mt-1 text-ink-100">{m.content}</p>
                <p className="mt-2 text-sm text-ink-400">{m.why_it_matters}</p>
                <p className="mt-3 border-l-2 border-accent-dim pl-3 text-sm text-ink-200">
                  <span className="text-ink-500">You could have asked: </span>
                  {m.unlock_question}
                </p>
              </li>
            ))}
